...
class Place_Detail_Auto_Collector {
    ...
    private function init() {
        ...
        add_action('wp_ajax_get_collection_history', array($this, 'ajax_get_collection_history'));
    ...
    public function render_admin_page() {
        ...
        // 네이버 검색 API 키 저장 처리
        if (isset($_POST['save_naver_search_api']) && check_admin_referer('save_naver_search_api', 'naver_search_api_nonce')) {
            update_option('naver_search_client_id', sanitize_text_field($_POST['naver_search_client_id']));
            update_option('naver_search_client_secret', sanitize_text_field($_POST['naver_search_client_secret']));
            echo '<div class="notice notice-success"><p>네이버 검색 API 키가 저장되었습니다.</p></div>';
        }
        ...
        <div class="wrap">
            ...
            <div class="card">
                <h2>네이버 검색 API 설정</h2>
                <p class="description">네이버 개발자 센터의 검색 API 키를 입력하세요. (네이버 맵 API와는 다른 키입니다)</p>
                <form method="post" action="">
                    <?php wp_nonce_field('save_naver_search_api', 'naver_search_api_nonce'); ?>
                    <table class="form-table">
                        <tr>
                            <th>Client ID</th>
                            <td>
                                <input type="text" name="naver_search_client_id" value="<?php echo esc_attr(get_option('naver_search_client_id', '')); ?>" class="regular-text" />
                            </td>
                        </tr>
                        <tr>
                            <th>Client Secret</th>
                            <td>
                                <input type="text" name="naver_search_client_secret" value="<?php echo esc_attr(get_option('naver_search_client_secret', '')); ?>" class="regular-text" />
                            </td>
                        </tr>
                    </table>
                    <button type="submit" name="save_naver_search_api" class="button button-primary">저장</button>
                </form>
            </div>
                    ...
                    <table class="form-table">
                        <tr>
                            ...
                            <td>
                                ...
                                <button type="button" id="view-history" class="button" style="margin-left: 10px;">이력 보기</button>
                            </td>
                        </tr>
                        <tr>
                            <th>수집 옵션</th>
        ...
        jQuery(document).ready(function($) {
            ...
            $('#view-history').on('click', function() {
                ...
                $.ajax({
                    ...
                    data: {
                        ...
                        post_id: post_id,
                        nonce: '<?php echo wp_create_nonce('get_collection_history'); ?>'
                    },
                    success: function(response) {
                        if (response.success) {
            ...
            function showHistoryModal(history) {
                ...
                history.forEach(function(entry, index) {
                    modalHtml += `
                        <div style="margin-bottom: 20px; padding: 15px; background: #f5f5f5; border-radius: 5px;">
                            <h4 style="margin: 0 0 10px 0;">수집 ${history.length - index}</h4>
                ...
                // TB_show 사용하여 모달 표시
                tb_show('수집 이력', '#TB_inline?width=600&height=550&inlineId=history-modal');
            }
    ...
    private function collect_from_naver($place_name, $address, $place_type) {
        ...
        // 네이버 검색 API용 별도 옵션 사용 (맵 API와 구분)
        $client_id = get_option('naver_search_client_id', '');
        $client_secret = get_option('naver_search_client_secret', '');
    ...
    private function collect_from_kakao($place_name, $address, $place_type) {
        ...
        // 카카오 로컬 API - 키워드 검색
        // 주소가 너무 구체적이면 검색 결과가 없을 수 있으므로 단순화
        $simple_address = $this->simplify_address($address);
        $search_query = $place_name . ' ' . $simple_address;
        ...
        error_log("카카오 검색어: " . $search_query);
        ...
        if (!empty($result['documents'])) {
            ...
            // 검색 결과가 없으면 더 간단한 검색어로 재시도
            if (strpos($search_query, '성수') !== false) {
                $retry_query = $place_name . ' 성수동';
                error_log("재시도 검색어: " . $retry_query);
                
                $retry_url = 'https://dapi.kakao.com/v2/local/search/keyword.json';
                $retry_url .= '?query=' . urlencode($retry_query);
                $retry_url .= '&size=5';
                
                $retry_response = wp_remote_get($retry_url, array(
                    'headers' => array(
                        'Authorization' => 'KakaoAK ' . $kakao_api_key
                    ),
                    'timeout' => 10
                ...
                if (!is_wp_error($retry_response)) {
                    ...
                    if (!empty($retry_result['documents'])) {
                        error_log("재시도 검색 결과: " . count($retry_result['documents']) . "개");
                        $best_match = $this->find_best_kakao_match($retry_result['documents'], $place_name);
     ...
     * 주소 단순화
     */
    private function simplify_address($address) {
        // 번지수 제거
        $address = preg_replace('/\d+[-]\d+/', '', $address);
        
        // 성수동만 추출
        if (preg_match('/성수[동]?[\d가-힣]*/', $address, $matches)) {
            return $matches[0];
        }
    ...
    public function ajax_collect_place_details() {
        ...
        try {
            ...
            wp_send_json_error('오류: ' . $e->getMessage());
        }
    }
     ...
     * AJAX: 수집 이력 가져오기
     */
    public function ajax_get_collection_history() {
        check_ajax_referer('get_collection_history', 'nonce');
        ...
        try {
            // 수집 이력 가져오기
            $history = get_post_meta($post_id, '_auto_collection_history', true);
            
            if (empty($history) || !is_array($history)) {
                wp_send_json_error('수집 이력이 없습니다.');
            }
            
            // PlaceMetaFields 클래스 로드
            if (!class_exists('PlaceMetaFields')) {
                $place_meta_fields_file = get_template_directory() . '/inc/places/place-meta-fields.php';
                if (file_exists($place_meta_fields_file)) {
                    require_once $place_meta_fields_file;
            ...
            foreach ($history as $entry) {
                ...
                // 필드명을 레이블로 변환
                foreach ($entry['fields_collected'] as $field_name) {
                    if (isset($field_definitions[$field_name])) {
                        $formatted_fields[] = array(
                            'name' => $field_name,
                            'label' => $field_definitions[$field_name]['label'],
                ...
                $formatted_history[] = array(
                    ...
                    'source' => isset($entry['source']) ? $entry['source'] : 'auto_collector',
                    'count' => count($entry['fields_collected']),
    ...
    private function test_google_api() {
        ...
        if (isset($data['status']) && $data['status'] === 'OK') {
            ...
            // 리퍼러 제한 오류인 경우 안내 메시지 추가
            if (strpos($error_message, 'referer restrictions') !== false) {
                $error_message = 'API 키 제한 설정 필요: 서버사이드 호출을 위해 IP 주소 제한 또는 제한 없음으로 설정하세요';
            }
